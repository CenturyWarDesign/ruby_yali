...
class AES
  def initialize()
    @AES=Rijndael.new(4)  
    key=[50, 117, 57, 104, 102, 115, 100, 106, 48, 111, 117, 113, 51, 119, 112, 122]
    @AES.load_ek key
    @AES.load_dk key
  end
  ...
  def cipher(input)
    ...
    while(arr.length>16)
      outArr += @AES.cipher(arr.shift(16))
    end
    if arr.length>0
      padding=16-arr.length
      (arr.length..15).each{
        arr.push(padding)
      }
      outArr += @AES.cipher(arr)
    end
    outStr=""
    outArr.each{
      |x|
    ...
    # 	puts c.ord
    # }
    return Base64.encode64(outStr).gsub(/\n/, '')
  end
